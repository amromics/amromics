...
def run_single_sample(sample, sample_dir, threads=8, memory=50, timing_log=None):
    ...
    if sample['input_type'] not in ['asm', 'assembly']:
        ...
        if len(pe_files) > 1:
            ...
            raise Exception('Only support pair-end reads!')
        sample['assembly'] =assembler.assemble_shovill(
            sample['id'],reads, base_dir=sample_dir, threads=threads, memory=memory, timing_log=timing_log)
    else:
        sample['assembly'] = assembler.get_assembly(sample['id'],sample['files'], base_dir=sample_dir)

    sample['annotation_gff'],sample['annotation_faa'],sample['annotation_ffn'],sample['annotation_fna'],sample['annotation_gbk'] = annotation.annotate_prokka(
        sample['id'], sample['assembly'],sample['genus'], sample['species'],sample['strain'],base_dir=sample_dir,  threads=threads, timing_log=timing_log)
    ...
    sample['virulome'] =amr.detect_virulome(
        sample['id'],sample['assembly'], base_dir=sample_dir, threads=threads, timing_log=timing_log)
    sample['plasmid'] = amr.detect_plasmid(
        sample['id'],sample['assembly'], base_dir=sample_dir, threads=threads, timing_log=timing_log)
